...
class JavaScriptExecutor @Inject constructor() {
    ...
    ///////////////
    fun cyclingVideo(webView: WebView, cycleVideo: Boolean) {
    ...
    //////////////
    fun videoFullScreen(webView: WebView) {
        webView.evaluateJavascript(
            "(function() { " +  // Ваш первый скрипт
                    ...
                    "}" +
                    "})();",
            null // this method make current video on the full screen(delete all except video container)
        )
    }

    fun videoFullScreenCancel(webView: WebView) {
        webView.evaluateJavascript(
            "(function() {" +
                    "var videos = document.querySelectorAll('video');" +
                    "videos.forEach(function(video) {" +
    ...
    /////////////
    fun makeSubtitleOf(webView: WebView) {
        webView.evaluateJavascript(
            "function disableSubtitles() {" +  // Method 1: Direct HTML5 video track disabling
                    ...
                    "  }" +
                    "}" +  // Run immediately
                    "disableSubtitles();" +  // Run periodically
                    "setInterval(disableSubtitles, 500);" +  // Also run when video source changes
                    "if(video) {" +
